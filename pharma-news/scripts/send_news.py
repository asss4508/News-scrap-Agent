...
def fetch_yakup(limit=8):
    ...
    articles = []
    # 기사 링크 추출
    for a in soup.select("ul.articles li a, div.news_list a, .article_list a, a"):
        href = a.get("href", "")
...
def fetch_pharmnews(limit=7):
    ...
    articles = []
    for a in soup.select("h4.titles a, h3 a, .article-list a, a"):
        href = a.get("href", "")
        title = a.get_text(strip=True)
...
def build_message(yakup_news, pharmnews_news):
    ...
    lines = [f"📰 *제약·바이오 뉴스브리핑*", f"_{today} 오전 8시_\n"]
